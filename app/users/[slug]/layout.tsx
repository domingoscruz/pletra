...
/**
 * Formats watch time minutes into days and hours.
 */
...
export default async function UserProfileLayout({ params, children }: Props) {
  ...
  try {
    // Fetch profile and stats in parallel using the authenticated client
    ...
    return (
      <>
        ...
        <div className="relative z-10 mx-auto max-w-[96rem] px-4 pt-6 pb-20 md:px-6">
    ...
    console.error("[Profile Layout] Failed to fetch profile:", error);
    return (
      <div className="flex min-h-[50vh] flex-col items-center justify-center text-center px-4">
        <h2 className="text-xl font-bold text-zinc-200">Unable to load profile</h2>
        <p className="mt-2 text-sm text-zinc-500">
          We encountered an issue while connecting to Trakt. Please try again later.
        </p>
        <Link href="/" className="mt-6 text-sm font-medium text-white underline underline-offset-4">
          Return Home
        </Link>
      </div>
    );
