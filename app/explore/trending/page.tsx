...
export default async function TrendingPage({ searchParams }: Props) {
  ...
  try {
    ...
    const [moviesRes, showsRes] = await Promise.all([
      fetchMovies
        ? client.movies.trending({ query })
        : Promise.resolve({ status: 200 as const, body: [], headers: new Headers() }),
      fetchShows
        ? client.shows.trending({ query })
        : Promise.resolve({ status: 200 as const, body: [], headers: new Headers() }),
    ]);

    const movies = moviesRes.status === 200 ? (moviesRes.body as unknown as TrendingMovie[]) : [];
    const shows = showsRes.status === 200 ? (showsRes.body as unknown as TrendingShow[]) : [];

    const moviePages = parseInt(
      String(moviesRes.headers.get?.("x-pagination-page-count") ?? "1"),
      10,
    );
    const showPages = parseInt(
      String(showsRes.headers.get?.("x-pagination-page-count") ?? "1"),
      10,
    );
    totalPages = Math.max(moviePages, showPages);

    const allItems = [
      ...movies.map((m) => ({ type: "movies" as const, data: m.movie, watchers: m.watchers })),
      ...shows.map((s) => ({ type: "shows" as const, data: s.show, watchers: s.watchers })),
    ...
    if (!isTraktExpectedError(error)) {
      console.error("[Explore] Trending page error:", error);
    }
  }
