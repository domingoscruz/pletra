  ...
  return withLocalCache(
    ...
    async () => {
      ...
      if (epRatingsRes?.status === 200) {
        (epRatingsRes.body as Array<{ episode?: { ids?: TraktIds }; rating: number }>).forEach(
          (r) => {
      ...
      if (movieRatingsRes?.status === 200) {
        (movieRatingsRes.body as Array<{ movie?: { ids?: TraktIds }; rating: number }>).forEach(
          (r) => {
      ...
      showSeasonsRes.forEach((res, i) => {
        ...
        const seasonsData = res.body as Array<{ number: number; aired_episodes?: number | null }>;
        ...
        seasonsData.forEach((s) => {
          if (s.number > 0) {
            episodesPerSeason[s.number] = s.aired_episodes || 0;
            if (s.number > maxS) maxS = s.number;
          }
        });
