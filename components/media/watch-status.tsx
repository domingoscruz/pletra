...
export function WatchStatus({
  ...
  const markWatched = useMutation({
    mutationFn: async () => {
      try {
        return await fetchTraktRouteJson("/api/trakt/sync/history", {
          method: "POST",
        ...
        throw new Error(getErrorMessage(error, "Failed to mark as watched"));
      }
    },
    onMutate: () => {
      setWatched(true);
    ...
    onError: (error) => {
      ...
      toast(getErrorMessage(error, "Failed to mark as watched"));
    },
  });
  ...
  const removeWatch = useMutation({
    mutationFn: async () => {
      try {
        return await fetchTraktRouteJson("/api/trakt/sync/history/remove", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            [mediaType]: [{ ids }],
        ...
        throw new Error(getErrorMessage(error, "Failed to remove from history"));
      }
    },
    onMutate: () => {
    ...
    onError: (error, _vars, context) => {
      ...
      toast(getErrorMessage(error, "Failed to remove from history"));
    },
  });
