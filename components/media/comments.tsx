...
export function Comments({
  ...
  const fetchComments = useCallback(
    async (p: number, s: SortMode) => {
      ...
      try {
        ...
        const data =
          (await fetchTraktRouteJson<CommentData[]>(
            `${path}?page=${p}&limit=${limit}&extended=full`,
            {
              timeoutMs: 10000,
              maxRetries: 2,
            },
  ...
  const submitComment = useCallback(async () => {
    ...
    try {
      ...
      const posted = await fetchTraktRouteJson<CommentData>("/api/trakt/comments", {
        method: "POST",
      ...
      toast(
        message.toLowerCase().includes("least 5 words")
